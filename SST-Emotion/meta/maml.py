...
Uses functional_call to run the model with custom parameter dicts, allowing
gradients to flow through inner-loop updates back to the meta-initialization
(second-order).

PyTorch < 2.0: torch.nn.utils._stateless.functional_call (takes a merged dict)
PyTorch >= 2.0: torch.func.functional_call (accepts (params, buffers) tuple)
...
try:
    from torch.func import functional_call as _fc_new

    def _functional_call(module, params, buffers, args):
        return _fc_new(module, (params, buffers), args)

except ImportError:
    from torch.nn.utils._stateless import functional_call as _fc_old

    def _functional_call(module, params, buffers, args):
        return _fc_old(module, {**params, **buffers}, args)
    ...
    def meta_train(
        ...
        for iteration in range(meta_iterations):
            ...
            for _ in range(inner_steps):
                ...
                out = _functional_call(self.model, task_params, buffers,
            ...
            out_q = _functional_call(self.model, task_params, buffers,
                                     (spec_q, temp_q))
